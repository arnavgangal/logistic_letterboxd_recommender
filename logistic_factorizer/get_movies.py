...
async def fetch_letterboxd(url, session, input_data={}):
    async with session.get(url) as r:
        ...
        # Parse ratings page response for each rating/review, use html.parser instead of lxml
...
async def get_rich_data(movie_list, db_cursor, mongo_db, tmdb_key):
    ...
    async with ClientSession() as session:
        ...
        for movie in movie_list:
            ...
            task = asyncio.ensure_future(fetch_tmdb_data(base_url.format(movie["tmdb_id"], tmdb_key), session, movie,
...
def main(data_type="letterboxd"):
    ...
    # Find all movies with missing metadata - they were added during get_ratings and have not been scraped yet
    # All other movies have already been scraped and
    # this is almost always unchanging data, we won't scrape 200,000+ records again
    if data_type == "letterboxd":
        newly_added = [x['movie_id'] for x in list(movies.find({"tmdb_id": {"$exists": False}}))]
        needs_update = [x['movie_id'] for x in
                        list(movies.find({"tmdb_id": {"$exists": True}}).sort("last_updated", -1).limit(6000))]
        all_movies = needs_update + newly_added
    elif data_type == "poster":
        two_months_ago = datetime.datetime.now() - datetime.timedelta(days=60)
        all_movies = [x['movie_id'] for x in list(
            movies.find({"$or": [{"image_url": {"$exists": False}}, {"last_updated": {"$lte": two_months_ago}}]}))]
    else:
        all_movies = [x for x in list(
            movies.find({"genres": {"$exists": False}, "tmdb_id": {"$ne": ""}, "tmdb_id": {"$exists": True}}))]
    ...
    for chunk_i in pbar:
        pbar.set_description(f"Scraping chunk {chunk_i + 1} of {num_chunks} in {data_type} scrape")
