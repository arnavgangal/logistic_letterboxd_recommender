...
if os.getcwd().endswith("/logistic_factorizer"):
    ...
    from logistic_factorizer.get_ratings import get_user_ratings


def get_page_count(username):
    """ get the number of pages of a user

    :param username: (str) username
    :return: (num_pages, display name of user)
    """
    ...
    try:
        ...
        display_name = body.find("section", attrs={"id": "profile-header"}).find("h1", attrs={
...
def get_user_data(username, data_opt_in=False):
    """ Gets all ratings data from a specific user

    :param username: string with username
    :param data_opt_in: boolean, whether to add user data to the database
    :return:
    """
    ...
    future = asyncio.ensure_future(
        get_user_ratings(username, db_cursor=None, mongo_db=None, store_in_db=False, num_pages=num_pages))
    loop.run_until_complete(future)

    # Store rating values, send to db if necessary
